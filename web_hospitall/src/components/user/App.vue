<template>
        <div id="main">
        <header id="header" class="header">
            <div class="wrapper header__content">
                <div class="header__icon">
                    <span class="bx bx-menu"></span>
                </div>
                <div class="header__logo">
                    <a href="">
                        <img src="https://bookingcare.vn/assets/icon/bookingcare-2020.svg" alt="" srcset="">
                    </a>
                </div>
                <ul class="header__list">
                    <li>
                        <p>Chuyên khoa</p>
                        <p>Tìm bác sỹ theo chuyên khoa</p>
                    </li>
                    <li>
                        <p>Chuyên khoa</p>
                        <p>Tìm bác sỹ theo chuyên khoa</p>
                    </li>
                    <li>
                        <p>Chuyên khoa</p>
                        <p>Tìm bác sỹ theo chuyên khoa</p>
                    </li>
                    <li>
                        <p>Chuyên khoa</p>
                        <p>Tìm bác sỹ theo chuyên khoa</p>
                    </li>
                </ul>
                <div class="header__support">
                    <i class='bx bx-support'></i>
                    <span>Hổ trợ</span>
                </div>
            </div>
            <div class="header__menu">
                <ul>
                    <li><a href="">
                            Trang chủ
                        </a></li>
                    <li><a href="">
                            Trang chủ
                        </a></li>
                    <li><a href="">
                            Trang chủ
                        </a></li>
                    <li>
                        <div>VỀ BOOKINGCARE</div>
                    </li>
                    <li>
                        <a href="">Về cho bệnh nhân</a>
                    </li>
                    <li>
                        <a href="">Về cho bác sỹ</a>
                    </li>
                </ul>
            </div>
            <div class="header__menu--overlay">
            </div>
        </header>
        <div class="banner">
            <div class="banner__image img-wrap">
                <img src="https://bookingcare.vn/assets/anh/bookingcare-cover-4.jpg" alt="">
            </div>
            <div class="banner__search">
                <h2>NỀN TẢNG Y TẾ</h2>
                <h1>CHĂM SÓC SỨC KHỎE TOÀN DIỆN</h1>
                <div class="banner__search--input col-md-4">
                    <input type="text" placeholder="Tìm gói khám">
                    <i class="bx bx-search"></i>
                    <div class="banner__search--popup">
                        <ul>
                            <li class="active">Chuyên khoa</li>
                            <li>Cơ xương khớp</li>
                            <li>Thần kinh</li>
                            <li>Tiêu hoá</li>
                            <li>Cơ xương khớp</li>
                            <li>Thần kinh</li>
                            <li>Tiêu hoá</li>
                            <li>Cơ xương khớp</li>
                            <li>Thần kinh</li>
                            <li>Tiêu hoá</li>
                            <li>Cơ xương khớp</li>
                            <li>Thần kinh</li>
                            <li>Tiêu hoá</li>
                            <li>Cơ xương khớp</li>
                            <li>Thần kinh</li>
                            <li>Tiêu hoá</li>
                            <li>Cơ xương khớp</li>
                            <li>Thần kinh</li>
                            <li>Tiêu hoá</li>
                        </ul>
                    </div>
                </div>
                <div class="banner__search--app col-md-4">
                    <img src="https://bookingcare.vn/assets/icon/google-play-badge.svg" alt="" srcset="">
                    <img src="https://bookingcare.vn/assets/icon/app-store-badge-black.svg" alt="">
                </div>
            </div>
            <div class="banner__option--container">
                <div class="wrapper banner__option">
                    <a href="" class="banner__option--item">
                        <div class="">
                            <img src="https://cdn.bookingcare.vn/fo/2021/12/08/133537-khamchuyenkhoa.png" alt="">
                        </div>
                        <p>
                            <span>Khám</span>
                            <span>Chuyên khoa</span>
                        </p>
                    </a>
                    <a href="" class="banner__option--item">
                        <div class="">
                            <img src="https://cdn.bookingcare.vn/fo/2021/12/08/133537-khamchuyenkhoa.png" alt="">
                        </div>
                        <p>
                            <span>Khám</span>
                            <span>Chuyên khoa</span>
                        </p>
                    </a>
                    <a href="" class="banner__option--item">
                        <div class="">
                            <img src="https://cdn.bookingcare.vn/fo/2021/12/08/133537-khamchuyenkhoa.png" alt="">
                        </div>
                        <p>
                            <span>Khám</span>
                            <span>Chuyên khoa</span>
                        </p>
                    </a>
                    <a href="" class="banner__option--item">
                        <div class="">
                            <img src="https://cdn.bookingcare.vn/fo/2021/12/08/133537-khamchuyenkhoa.png" alt="">
                        </div>
                        <p>
                            <span>Khám</span>
                            <span>Chuyên khoa</span>
                        </p>
                    </a>
                    <a href="" class="banner__option--item">
                        <div class="">
                            <img src="https://cdn.bookingcare.vn/fo/2021/12/08/133537-khamchuyenkhoa.png" alt="">
                        </div>
                        <p>
                            <span>Khám</span>
                            <span>Chuyên khoa</span>
                        </p>
                    </a>
                    <a href="" class="banner__option--item">
                        <div class="">
                            <img src="https://cdn.bookingcare.vn/fo/2021/12/08/133537-khamchuyenkhoa.png" alt="">
                        </div>
                        <p>
                            <span>Khám</span>
                            <span>Chuyên khoa</span>
                        </p>
                    </a>
                    <a href="" class="banner__option--item">
                        <div class="">
                            <img src="https://cdn.bookingcare.vn/fo/2021/12/08/133537-khamchuyenkhoa.png" alt="">
                        </div>
                        <p>
                            <span>Khám</span>
                            <span>Chuyên khoa</span>
                        </p>
                    </a>
                    <a href="" class="banner__option--item">
                        <div class="">
                            <img src="https://cdn.bookingcare.vn/fo/2021/12/08/133537-khamchuyenkhoa.png" alt="">
                        </div>
                        <p>
                            <span>Khám</span>
                            <span>Chuyên khoa</span>
                        </p>
                    </a>
                </div>
            </div>
        </div>
        <div class="wrapper service">
            <div class="row">
                <div class="service__slider col-12">
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 service__slider--item">
                        <span class="service__slider--item--sticky">
                            COVID
                        </span>
                        <div class="service__slider--item--image">
                            <div class="img-wrap">
                                <img src="https://cdn.bookingcare.vn/fo/2022/01/21/170022-kit-test-nhanh.png" alt=""
                                    srcset="">
                            </div>
                        </div>
                        <h3>Kit Test COVID bằng nước bọt</h3>
                        <ul>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                        </ul>
                        <p>XEM CHI TIẾT <span class="bx bx-chevrons-right"></span></p>
                    </a>
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 service__slider--item">
                        <span class="service__slider--item--sticky">
                            COVID
                        </span>
                        <div class="service__slider--item--image">
                            <div class="img-wrap">
                                <img src="https://cdn.bookingcare.vn/fo/2022/01/21/170022-kit-test-nhanh.png" alt=""
                                    srcset="">
                            </div>
                        </div>
                        <h3>Kit Test COVID bằng nước bọt</h3>
                        <ul>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                        </ul>
                        <p>XEM CHI TIẾT <span class="bx bx-chevrons-right"></span></p>
                    </a>
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 service__slider--item">
                        <span class="service__slider--item--sticky">
                            COVID
                        </span>
                        <div class="service__slider--item--image">
                            <div class="img-wrap">
                                <img src="https://cdn.bookingcare.vn/fo/2022/01/21/170022-kit-test-nhanh.png" alt=""
                                    srcset="">
                            </div>
                        </div>
                        <h3>Kit Test COVID bằng nước bọt</h3>
                        <ul>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                        </ul>
                        <p>XEM CHI TIẾT <span class="bx bx-chevrons-right"></span></p>
                    </a>
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 service__slider--item">
                        <span class="service__slider--item--sticky">
                            COVID
                        </span>
                        <div class="service__slider--item--image">
                            <div class="img-wrap">
                                <img src="https://cdn.bookingcare.vn/fo/2022/01/21/170022-kit-test-nhanh.png" alt=""
                                    srcset="">
                            </div>
                        </div>
                        <h3>Kit Test COVID bằng nước bọt</h3>
                        <ul>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                        </ul>
                        <p>XEM CHI TIẾT <span class="bx bx-chevrons-right"></span></p>
                    </a>
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 service__slider--item">
                        <span class="service__slider--item--sticky">
                            COVID
                        </span>
                        <div class="service__slider--item--image">
                            <div class="img-wrap">
                                <img src="https://cdn.bookingcare.vn/fo/2022/01/21/170022-kit-test-nhanh.png" alt=""
                                    srcset="">
                            </div>
                        </div>
                        <h3>Kit Test COVID bằng nước bọt</h3>
                        <ul>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                        </ul>
                        <p>XEM CHI TIẾT <span class="bx bx-chevrons-right"></span></p>
                    </a>
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 service__slider--item">
                        <span class="service__slider--item--sticky">
                            COVID
                        </span>
                        <div class="service__slider--item--image">
                            <div class="img-wrap">
                                <img src="https://cdn.bookingcare.vn/fo/2022/01/21/170022-kit-test-nhanh.png" alt=""
                                    srcset="">
                            </div>
                        </div>
                        <h3>Kit Test COVID bằng nước bọt</h3>
                        <ul>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                            <li>Tầm soát và xác định COVID-19</li>
                        </ul>
                        <p>XEM CHI TIẾT <span class="bx bx-chevrons-right"></span></p>
                    </a>
                </div>
            </div>
        </div>
        <div class="wrapper">
            <h2 class="item__post--heading">
                Chuyên khoa phổ biến
            </h2>
            <div class="row">
                <div class="item__post">
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 item__post--item">
                        <div class="item__post--item--image img-wrap"
                            style="background: url('https://cdn.bookingcare.vn/fr/w300/2019/12/13/120331-co-xuong-khop.jpg');">
                        </div>
                        <p class="item__post--item--text">
                            Cơ Xương Khớp
                        </p>
                    </a>
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 item__post--item">
                        <div class="item__post--item--image img-wrap"
                            style="background: url('https://cdn.bookingcare.vn/fr/w300/2019/12/13/120331-co-xuong-khop.jpg');">
                        </div>
                        <p class="item__post--item--text">
                            Cơ Xương Khớp
                        </p>
                    </a>
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 item__post--item">
                        <div class="item__post--item--image img-wrap"
                            style="background: url('https://cdn.bookingcare.vn/fr/w300/2019/12/13/120331-co-xuong-khop.jpg');">
                        </div>
                        <p class="item__post--item--text">
                            Cơ Xương Khớp
                        </p>
                    </a>
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 item__post--item">
                        <div class="item__post--item--image img-wrap"
                            style="background: url('https://cdn.bookingcare.vn/fr/w300/2019/12/13/120331-co-xuong-khop.jpg');">
                        </div>
                        <p class="item__post--item--text">
                            Cơ Xương Khớp
                        </p>
                    </a>
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 item__post--item">
                        <div class="item__post--item--image img-wrap"
                            style="background: url('https://cdn.bookingcare.vn/fr/w300/2019/12/13/120331-co-xuong-khop.jpg');">
                        </div>
                        <p class="item__post--item--text">
                            Cơ Xương Khớp
                        </p>
                    </a>
                    <a href="" class="col-12 col-sm-6 col-md-4 col-lg-3 item__post--item">
                        <div class="item__post--item--image img-wrap"
                            style="background: url('https://cdn.bookingcare.vn/fr/w300/2019/12/13/120331-co-xuong-khop.jpg');">
                        </div>
                        <p class="item__post--item--text">
                            Cơ Xương Khớp
                        </p>
                    </a>
                </div>
            </div>
        </div>
        <div class="wrapper">
            <p class="item__post--heading">
                Cẩm nang
            </p>
            <div class="row">
                <div class="item__guide">
                    <a href="" class="col-12 row col-md-6 item__guide--item">
                        <div class="item__guide--item--image col-4 img-wrap"
                            style="background-image: url('https://cdn.bookingcare.vn/fr/w300/2022/05/06/102403-benh-vien-nam-hoc-va-hiem-muon-ha-noi.jpg');">
                        </div>
                        <p class="item__guide--item--text col-8">
                            Tuần lễ vàng - Ưu đãi tháng 5 tại Bệnh Viện Nam học và Hiếm muộn Hà Nội
                        </p>
                    </a>
                    <a href="" class="col-12 row col-md-6 item__guide--item">
                        <div class="item__guide--item--image col-4 img-wrap"
                            style="background-image: url('https://cdn.bookingcare.vn/fr/w300/2022/05/06/102403-benh-vien-nam-hoc-va-hiem-muon-ha-noi.jpg');">
                        </div>
                        <p class="item__guide--item--text col-8">
                            Tuần lễ vàng - Ưu đãi tháng 5 tại Bệnh Viện Nam học và Hiếm muộn Hà Nội
                        </p>
                    </a>
                    <a href="" class="col-12 row col-md-6 item__guide--item">
                        <div class="item__guide--item--image col-4 img-wrap"
                            style="background-image: url('https://cdn.bookingcare.vn/fr/w300/2022/05/06/102403-benh-vien-nam-hoc-va-hiem-muon-ha-noi.jpg');">
                        </div>
                        <p class="item__guide--item--text col-8">
                            Tuần lễ vàng - Ưu đãi tháng 5 tại Bệnh Viện Nam học và Hiếm muộn Hà Nội
                        </p>
                    </a>
                    <a href="" class="col-12 row col-md-6 item__guide--item">
                        <div class="item__guide--item--image col-4 img-wrap"
                            style="background-image: url('https://cdn.bookingcare.vn/fr/w300/2022/05/06/102403-benh-vien-nam-hoc-va-hiem-muon-ha-noi.jpg');">
                        </div>
                        <p class="item__guide--item--text col-8">
                            Tuần lễ vàng - Ưu đãi tháng 5 tại Bệnh Viện Nam học và Hiếm muộn Hà Nội
                        </p>
                    </a>
                </div>
            </div>
        </div>
        <footer class="footer" id="footer">
            <div class="footer__main">
                <div class="wrapper">
                    <ul>
                        <li>
                            <div class="footer__main--logo">
                                <img src="https://bookingcare.vn/assets/icon/bookingcare-2020.svg" alt="">
                            </div>
                        </li>
                        <li>Công ty Cổ phần Công nghệ BookingCare</li>
                        <li>
                            <i class='bx bx-current-location'></i>
                            <p>28 Thành Thái, Dịch Vọng, Cầu Giấy, Hà Nội</p>
                        </li>
                        <li>
                            <i class="bx bx-check"></i>
                            <p>ĐKKD số: 0106790291. Sở KHĐT Hà Nội cấp ngày 16/03/2015</p>
                        </li>
                        <li>
                            <img src="https://bookingcare.vn/assets/icon/bo-cong-thuong.svg" alt="">
                            <img src="https://bookingcare.vn/assets/icon/bo-cong-thuong.svg" alt="">
                        </li>
                    </ul>
                    <ul>
                        <li>
                            <a href="">Liên hệ hợp tác</a>
                        </li>
                        <li>
                            <a href="">Câu hỏi thường gặp</a>
                        </li>
                        <li>
                            <a href="">Điều khoản sử dụng</a>
                        </li>
                        <li>
                            <a href="">Chính sách Bảo mật</a>
                        </li>
                        <li>
                            <a href="">Quy trình hỗ trợ giải quyết khiếu nại</a>
                        </li>
                        <li>
                            <a href="">Quy chế hoạt động</a>
                        </li>
                    </ul>
                    <ul>
                        <li>
                            <h4>Trụ sở tại Hà Nội</h4>
                            <p>28 Thành Thái, Dịch Vọng, Cầu Giấy, Hà Nội</p>
                        </li>
                        <li>
                            <h4>Trụ sở tại Hà Nội</h4>
                            <p>28 Thành Thái, Dịch Vọng, Cầu Giấy, Hà Nội</p>
                        </li>
                        <li>
                            <h4>Trụ sở tại Hà Nội</h4>
                            <p>28 Thành Thái, Dịch Vọng, Cầu Giấy, Hà Nội</p>
                        </li>
                    </ul>
                </div>
            </div>
            <div class="footer__end">
                <div class="wrapper footer__end--main">
                    <span>@20222 BookingCare</span>
                    <div>
                        <a href="" class="bx bxl-facebook"></a>
                        <a href="" class="bx bxl-youtube"></a>
                    </div>
                </div>
            </div>
        </footer>
    </div>
</template>

<style scoped src = '@/assets/style/user/app.css'></style>